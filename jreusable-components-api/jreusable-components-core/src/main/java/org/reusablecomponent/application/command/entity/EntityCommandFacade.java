...
import org.reusablecomponent.infra.exception.ElementWithIdNotFoundException;
import org.reusablecomponent.infra.messaging.event.Event;
import org.reusablecomponent.infra.messaging.event.OperationEvent;
...
public class EntityCommandFacade<Entity extends AbstractEntity<Id>, Id, OneResult, MultipleResult, VoidResult, BooleanResult> // Operations on Entity 
	...
	if (Objects.equals(FALSE, existsEntity)) {
	    throw new ElementWithIdNotFoundException(getEntityClazz(), getI18nFunction(), id);
	}
	...
	if (Objects.equals(FALSE, existsEntity)) {
	    throw new ElementWithIdNotFoundException(getEntityClazz(), getI18nFunction(), entity.getId());
	}
     ...
     */
    @Override
    public VoidResult deleteAllBy(@NotNull final Iterable<@NotNull Id> ids) {
	preDeleteBy(ids);
