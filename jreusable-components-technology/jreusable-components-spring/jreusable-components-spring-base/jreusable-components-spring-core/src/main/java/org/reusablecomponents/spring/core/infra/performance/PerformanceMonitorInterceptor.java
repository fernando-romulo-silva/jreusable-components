 ...
 * slow or not. <br />
 * The main idea is to log informations about slow executions on critical
 * methods. <br />
 ...
 * </pre>
 * 
 * To connect this interceptor with an aspect advisor, please access (@link
 * PerformanceMonitorConfiguration} <br />
 * <br />
 * 
 * The log level (OFF, TRACE, DEBUG, INFO, etc) is defined by the time of
 * intercepted method spent. <br />
 * For example, an intercepted method tha spent between 0 and 500 milliseconds
 * shows a WARN level. <br />
...
public class PerformanceMonitorInterceptor implements MethodInterceptor {
	...
	private AbstractMap.SimpleEntry<String, String> findCallerAndStack() {
		...
		final var stackList = StackWalker.getInstance()
				...
				.map(trace -> substringBefore(trace.getClassName(), "$$EnhancerBySpringCGLIB") + "."
	...
	/**
	 * Disable the interceptor, and now it's going to skip the performance
	 * measuring.
	 * 
	 * @return returns itself
	 */
