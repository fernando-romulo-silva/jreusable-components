...
public class SpringEntityQueryHttpController<Entity extends AbstractEntity<Id>, Id>
		extends EntityQueryHttpController<Entity, Id, //
				...
				ResponseEntity<Void>, ResponseEntity<Entity>, ResponseEntity<Iterable<Entity>>>
		implements InterfaceSpringEntityQueryHttpController<Entity, Id> {

	protected SpringEntityQueryHttpController(final InterfaceSpringQueryFacade<Entity, Id> entityQueryFacade) {
		super(new EntityQueryHttpControllerBuilder<>($ -> {
			...
			$.createResponseHeadFunction = (exists) -> Objects.equals(TRUE, exists) ? ResponseEntity.ok().build()
